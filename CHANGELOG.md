...
- Fix `advpcd!` for `CenteredRBM` erroring with RestrictedBoltzmannMachines 6.2 (`UndefVarError: grad2ave`): the hidden-offset update now estimates `<h>` from the minibatch via `center_hidden_from_data!` and its `damping` keyword, like the upstream centered trainer, instead of reading it off the free-energy gradient with the removed `grad2ave`. RestrictedBoltzmannMachines 6.2 or later is now required (was 5.6).
